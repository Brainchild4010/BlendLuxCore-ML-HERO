...
class LUXCORE_RENDER_PT_ml_hero(RenderButtonsPanel, Panel):
    COMPAT_ENGINES = {"LUXCORE"}
    bl_label = "ML HERO Spectral"
    bl_order = 15
...
class LUXCORE_RENDER_PT_lightpaths_bounces(RenderButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        if config.engine == "PATH":
            ...
            col.separator()
            col.prop(config.path, "rr_depth", text="Russian Roulette Start Depth")
            col.prop(config.path, "rr_cap", text="Russian Roulette Survival Limit")
        else:
            # Bidir options
            ...
            col.separator()
            col.prop(config, "bidir_rr_depth", text="Russian Roulette Start Depth")
            col.prop(config, "bidir_rr_cap", text="Russian Roulette Survival Limit")

...
def _has_ml_custom_pyluxcore():
    """Return True when our custom ML pyluxcore build is already installed.

    Custom builds expose pyluxcore.MLVersion(). In that case BlendLuxCore must
    not replace the installed package with the official PyPI wheel.
    """
    try:
        import pyluxcore
    except Exception as err:
        print(f"[BLC] ML custom pyluxcore check failed: {err}")
        return False

    ml_version = getattr(pyluxcore, "MLVersion", None)
    if not callable(ml_version):
        return False

    try:
        version = ml_version()
    except Exception as err:
        print(f"[BLC] MLVersion() check failed: {err}")
        return False

    print(f"[BLC] ML custom pyluxcore detected: {version}")
    print("[BLC] Skipping automatic pyluxcore wheel replacement")
    return True
...
def ensure_pyluxcore():
    ...
    # Protect our custom ML build from BlendLuxCore's automatic wheel updater.
    # If MLVersion() exists, pyluxcore is our locally built version and must
    # not be replaced by the official PyPI wheel.
    if _has_ml_custom_pyluxcore():
        return

...
def convert(exporter, scene, context=None, engine=None):
    try:
        ...
        pipeline_index += 1
        add_DENOISER_AOVs = ((final and denoiser.enabled and denoiser.type in {"OIDN", "OPTIX"})
...
def get_denoiser_imgpipeline_props(context, scene, pipeline_index):
    ...
    elif scene.luxcore.denoiser.type == "OPTIX":
        index = get_OPTIX_props(definitions, scene, index)
...
def get_OPTIX_props(definitions, scene, index):
    denoiser = scene.luxcore.denoiser
    definitions[str(index) + ".type"] = "OPTIX_DENOISER"
    definitions[str(index) + ".minspp"] = denoiser.optix_min_samples
    definitions[str(index) + ".sharpness"] = denoiser.optix_sharpness
    return index + 1
